      ...
      result.values.forEach((row) => {
        ...
        data.push({
          ...
          age: Math.abs(
            Math.floor(
              DateTime.now().diff(
                DateTime.fromJSDate(new Date(row[result.columns.indexOf("birthyear")])),
                "years",
              ).years,
            ),
          ),
...
function calcRetardTime() {
  ...
  const startTimeToday = now.set({ hour: startTime.hour, minute: startTime.minute });
...
function getDatesInRange() {
  ...
  if (result.length && result[0].values.length) {
    ...
    const startDate = DateTime.fromJSDate(selectedDates[0]).startOf("day").toJSDate();
    const endDate = DateTime.fromJSDate(selectedDates[1]).startOf("day").toJSDate();
